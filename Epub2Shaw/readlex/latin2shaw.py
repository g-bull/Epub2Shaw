...
def latin2shaw(text, extra_words_json_filename=None, extra_phrases_json_filename=None):
    ...
    if extra_words_json_filename is not None:
        with open(extra_words_json_filename, 'r', encoding="utf-8") as file:
            json_data = file.read()
            extra_dict: dict[str, list[dict[str, str]]] = json.loads(json_data)
            readlex_dict = {**readlex_dict, **extra_dict}
